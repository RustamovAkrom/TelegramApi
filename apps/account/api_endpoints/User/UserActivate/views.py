        ...
        if phone_number and token:
            ...
            if user_exist:
                ...
                if not account_exists:
                    ...
                    user_account_link = "http://127.0.0.1:8000" + reverse(
                        "account:user-account-retrive", kwargs={"pk": account.pk}
                    )

                    return Response(
                        data={
                            "detail": _(
                                f"Successfully activated user account, Your account link -> {user_account_link}"
                            )
                        },
            ...
            return Response(
                data={"detail": _(f"User not found!")}, status=status.HTTP_404_NOT_FOUND
            )
        return Response(
            data={"detail": _("<Phone number> or <token> not found !")},
