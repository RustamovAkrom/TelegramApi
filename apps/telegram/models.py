...
class ChatUser(BaseSharedModel):
    user = models.OneToOneField(
        "account.UserAccount", models.CASCADE, related_name="user_chat"
    )
...
class ChatMessage(BaseSharedModel):
    ...
    media = models.FileField(
        _("media"), upload_to="telegram/chat/messages/%Y/%m/%d", blank=True, null=True
    )
...
class Chat(BaseSharedModel):
    user_from = models.ForeignKey(
        ChatUser, models.CASCADE, related_name="user_from_chat"
    )
...
class GroupUser(BaseSharedModel):
    user = models.ForeignKey(
        "account.UserAccount", models.CASCADE, related_name="user_group"
    )
...
class GroupMessage(BaseSharedModel):
    user = models.ForeignKey(
        GroupUser, models.CASCADE, related_name="group_user_messages"
    )
    message = models.TextField(_("message"))
    media = models.FileField(
        _("media"), upload_to="telegram/groups/messages/%Y/%m/%d", blank=True, null=True
    )
...
class Group(BaseSharedModel):
    ...
    public_link = models.CharField(
        _("public link"), max_length=120, blank=True, null=True, unique=True
    )
...
class ChannelUser(BaseSharedModel):
    user = models.ForeignKey(
        "account.UserAccount", models.CASCADE, related_name="user_channel"
    )
    is_owner = models.BooleanField(_("is owner"), default=False)
    is_admin = models.BooleanField(_("is admin"), default=False)
    is_active = models.BooleanField(_("is active"), default=True)
...
class ChannelStories(BaseSharedModel):
    storie = models.FileField(
        _("storie"), upload_to="telegram/channel/stories/%Y/%m/%d"
    )
...
class ChannelMessage(BaseSharedModel):
    user = models.ForeignKey(
        ChannelUser, models.CASCADE, related_name="channel_user_messages"
    )
    message = models.TextField(_("message"))
    media = models.FileField(
        _("media"),
...
class Channel(BaseSharedModel):
    ...
    stories = models.ManyToManyField(
        ChannelStories, related_name="stories_channel", blank=True
    )
